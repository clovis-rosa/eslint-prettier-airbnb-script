...
> P.S.: This is an adjustment for restore compatibility with create-react-app and gatsby-cli.<br>
> It will be using the ESLint package version already included in those boilerplates.
...
2. Run this command inside your app's root directory. Note: this command executes the `eslint-prettier-config.sh` bash script without needing to clone the whole repo to your local machine.
   ...
   ```bash
   exec 3<&1;bash <&3 <(curl https://raw.githubusercontent.com/clovis-rosa/eslint-prettier-airbnb-script/master/eslint-prettier-config.sh 2> /dev/null)
   ```
